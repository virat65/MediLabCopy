...
function Service() {
  ...
  const handleAppointmentClick = (doctor) => {
    ...
    if (!user) {
      ...
      // Doctor ki id pass karte hain appointment page pe
  ...
  useEffect(() => {
    if (selectedService) {
      axios
        .get(
          `http://localhost:3333/doctors-by-specialization?specialization=${selectedService}`
        )
        .then((res) => {
                  ...
                  <div className="card">
                    <div className="card-body d-flex align-items-center">
                      <img
                        src={
                          doc.image
                            ? `http://localhost:3333/images/userImage/${doc.image}`
